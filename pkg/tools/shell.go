	...
	if config != nil {
		...
		if enableDenyPatterns {
			if len(execConfig.CustomDenyPatterns) > 0 {
				logger.InfoCF("exec", "Using custom deny patterns", map[string]interface{}{"patterns": execConfig.CustomDenyPatterns})
				for _, pattern := range execConfig.CustomDenyPatterns {
					re, err := regexp.Compile(pattern)
					if err != nil {
						logger.ErrorCF("exec", "Invalid custom deny pattern", map[string]interface{}{"pattern": pattern, "error": err.Error()})
						continue
					}
			...
			logger.WarnC("exec", "deny patterns are disabled. All commands will be allowed.")
		}
	} else {
		denyPatterns = append(denyPatterns, defaultDenyPatterns...)
	}
